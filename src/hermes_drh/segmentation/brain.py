...
def save_fifteen_slice_modality_montages(
    volume_4d,
    segmentation_3d,
    output_base,
    images_dir,
    ...
    """
    Write five 3×5 PNG montages (15 axial slices each): FLAIR, T1, T1CE, T2, and
    FLAIR + segmentation overlay. Uses the same slice indices for every figure
    (e.g. top-15 by tumor area from ``select_slices_biggest_tumor``).
    """
    try:
        import matplotlib.pyplot as plt
        from matplotlib.colors import ListedColormap
    ...
    if volume_4d is None or segmentation_3d is None:
        return []
    os.makedirs(images_dir, exist_ok=True)
    n = min(15, len(slice_indices))
    if n == 0:
        return []
    use_idx = list(slice_indices[:n])
    seg_vmax = 4 if np.max(segmentation_3d) >= 4 else 3
    seg_cmap = ListedColormap(SEG_COLORS_STREAMLIT[: seg_vmax + 1])
    modality_labels = [
        ("flair", 0, "FLAIR"),
        ("t1", 1, "T1"),
        ("t1ce", 2, "T1CE"),
    ...
    def _save_montage(fig, suffix: str):
        path = os.path.join(
            images_dir, f"{output_base}_modality_montage_{suffix}_3x5.png"
        )
        fig.savefig(path, dpi=120, bbox_inches="tight")
        plt.close(fig)
        out_paths.append(path)

    for suffix, ch, title_prefix in modality_labels:
        ...
        for i in range(15):
            ...
            if i < n:
                z = use_idx[i]
                img_slice = volume_4d[ch, :, :, z]
                ax.imshow(img_slice, cmap="gray", origin="lower")
                ax.set_title(f"{title_prefix} z={z}", fontsize=8)
            ax.axis("off")
        fig.suptitle(
            f"{output_base} — {title_prefix}: top {n} axial slices by tumor area (3×5)",
            fontsize=11,
        )
        plt.tight_layout()
        _save_montage(fig, suffix)
    ...
    for i in range(15):
        ...
        if i < n:
            z = use_idx[i]
            base_img = volume_4d[0, :, :, z]
            seg_slice = segmentation_3d[:, :, z]
            ax.imshow(base_img, cmap="gray", origin="lower", alpha=0.7)
            mask_overlay = np.ma.masked_where(seg_slice == 0, seg_slice)
            ax.imshow(
                mask_overlay,
                cmap=seg_cmap,
            ...
            ax.set_title(f"FLAIR + seg z={z}", fontsize=8)
        ax.axis("off")
    fig.suptitle(
        f"{output_base} — FLAIR + segmentation overlay: top {n} axial slices (3×5)",
        fontsize=11,
    )
    plt.tight_layout()
    _save_montage(fig, "flair_seg_overlay")
    return out_paths
