...
def regenerate_run_figures_from_disk(results_dir: str | Path, output_base: str) -> bool:
    ...
    try:
        if e0.is_file() and meta_path.is_file():
            from build_animations_from_streamed_frames import FrameLoader
            from multiview_visualization import export_all_multiview_static
            ...
            from multiview_visualization import export_multiview_plates_from_volumes
...
# write_progress imported from progress.py (single responsibility: dashboard progress)
...
def build_and_save_animations(
    ...
    if stream_frames and streamed_n_frames > 0:
        ...
        anim_script = os.path.join(
            script_dir, "build_animations_from_streamed_frames.py"
        )
        if os.path.isfile(anim_script):
            anim_argv = [
                sys.executable,
                anim_script,
                results_dir_abs,
            ...
            proc = subprocess.run(anim_argv, cwd=script_dir, stdin=subprocess.DEVNULL)
            if proc.returncode == 0:
                print(
                    f"  build_animations_from_streamed_frames.py finished (saved to {animations_dir_abs})."
                )
            else:
                print(
            ...
            print(
                f"  Warning: {anim_script} not found; run build_animations_from_streamed_frames.py manually."
            )
        return time.perf_counter() - t_start_anim
    if len(E_frames) == 0:
        return time.perf_counter() - t_start_anim
